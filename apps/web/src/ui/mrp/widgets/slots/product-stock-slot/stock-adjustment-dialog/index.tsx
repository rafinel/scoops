        ...
        <DialogHeader className='flex-row items-start gap-3 border-b border-border-soft p-5 pr-14 sm:p-6 sm:pr-14'>
          ...
          <div className='min-w-0'>
            ...
            <DialogDescription className='mt-1'>
              {brand ? `${brand.brand.name} · ` : ''}Saldo atual:{' '}
              {formatQuantity(currentBalance, unit)}
            </DialogDescription>
          </div>
        </DialogHeader>
        ...
        <form noValidate onSubmit={handleSubmit}>
          <div className='grid gap-5 p-5 sm:p-6'>
            ...
            {inputMode === 'package' && brand ? (
              <div className='rounded-xl bg-primary-soft p-4 text-sm'>
                ...
                <p className='mt-1 text-muted-foreground'>
                  {quantity || '0'} × {formatQuantity(brand.brand.packageQuantity, unit)}{' '}
                  ={' '}
            ...
            >
              ...
              >
                {formatQuantity(prospectiveBalance, unit)}
              </p>
              {isInsufficient ? (
                <p className='mt-2 text-destructive' role='alert'>
                  Estoque insuficiente. Disponível: {formatQuantity(currentBalance, unit)}
                  ; solicitado: {formatQuantity(baseQuantity, unit)}.
                </p>
              ) : null}
            </div>
