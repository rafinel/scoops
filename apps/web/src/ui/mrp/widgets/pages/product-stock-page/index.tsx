...
import { ProductStockHeader } from './product-stock-header'
...
export type ProductStockPageProps = { productId: string }

export const ProductStockPage = ({ productId }: ProductStockPageProps) => {
  const {
    productStock,
    selectedAction,
  ...
  return (
    <section className='min-w-0 max-w-full space-y-5 overflow-hidden pb-8'>
      ...
      {productStock && !isLoading && !isError ? (
        <>
          <ProductStockHeader product={productStock.product} />
          <nav aria-label='Seção ativa' className='inline-flex rounded-xl bg-muted p-1'>
            <span className='flex items-center gap-2 rounded-lg bg-card px-4 py-2 text-sm font-bold text-primary shadow-sm'>
              <Icon className='size-4' name='package' /> Estoque
            </span>
          </nav>
          <ProductStockSummary
