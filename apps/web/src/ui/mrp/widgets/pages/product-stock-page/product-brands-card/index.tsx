...
export const ProductBrandsCard = ({
  ...
  return (
    <section className='min-w-0 rounded-2xl bg-card p-4 shadow-sm ring-1 ring-foreground/5 sm:p-6'>
      ...
      {rows.length > 0 ? (
        <div className='mt-5 hidden overflow-x-auto rounded-xl border border-border-soft lg:block'>
          <table className='w-full min-w-[900px] text-left text-sm'>
            <caption className='sr-only'>Estoque por marca</caption>
            <thead className='bg-muted/60 text-xs uppercase tracking-wide text-muted-foreground'>
              <tr>
                <th className='px-4 py-3 font-semibold'>Marca</th>
                <th className='px-4 py-3 font-semibold'>Qtd. embalagem</th>
                <th className='px-4 py-3 font-semibold'>Valor/embalagem</th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr className='border-t border-border-soft' key={row.brand.id}>
                  ...
                  <td className='px-4 py-4'>{row.formattedPackageQuantity}</td>
                  <td className='px-4 py-4'>{row.formattedPackagePrice}</td>
                  <td className='px-4 py-4'>{row.formattedUnitPrice}</td>
