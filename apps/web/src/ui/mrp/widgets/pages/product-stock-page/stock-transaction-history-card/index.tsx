    ...
    <section className='min-w-0 overflow-hidden rounded-2xl bg-card shadow-sm ring-1 ring-foreground/5'>
      ...
      {query.data && query.data.items.length > 0 ? (
        <>
          ...
          <div className='hidden overflow-x-auto border-y border-border-soft lg:block'>
            <table className='w-full min-w-[850px] text-left text-sm'>
              ...
              <thead className='bg-muted/60 text-xs uppercase tracking-wide text-muted-foreground'>
                <tr>
                  <th className='px-4 py-3 font-semibold sm:px-6'>Data/hora</th>
              ...
              <tbody>
                {query.data.items.map((transaction) => (
                  <tr className='border-t border-border-soft' key={transaction.id}>
                    ...
                    <td className='px-4 py-4'>{transaction.brandName ?? 'Produto'}</td>
                    <td className='px-4 py-4'>
                      <SignedQuantity
                        quantity={transaction.quantity}
                        type={transaction.type}
...
function TransactionType({ type }: { type: StockAdjustmentType }) {
  return (
    <Badge
      className={
        type === 'entry'
