...
class MrpSalesCatalogReader implements SalesCatalogProvider {
  constructor(
...
export class MrpSalesCatalogProvider implements SalesCatalogProvider {
  ...
  findProductIdsByName(
    ...
    return this.withReader((reader) =>
      reader.findProductIdsByName(establishmentId, search),
    )
  }
  ...
  findByProductIds(
    ...
    return this.withReader((reader) =>
      reader.findByProductIds(establishmentId, productIds),
    )
  }
  ...
  findByProductId(
    ...
    return this.withReader((reader) => reader.findByProductId(establishmentId, productId))
  }
  ...
  findMany(input: {
    ...
    return this.withReader((reader) => reader.findMany(input))
  }

  private withReader<Result>(
    operation: (reader: MrpSalesCatalogReader) => Promise<Result>,
  ): Promise<Result> {
    return this.database.run((repositories) => operation(this.createReader(repositories)))
  }

  private createReader(repositories: MrpDatabaseRepositories): MrpSalesCatalogReader {
    return new MrpSalesCatalogReader(
      repositories.productsRepository,
      repositories.productSizesRepository,
      repositories.productAccompanimentsRepository,
      repositories.accompanimentTypesRepository,
      repositories.resaleConfigurationsRepository,
      repositories.brandsRepository,
      repositories.stockBalancesRepository,
    )
  }
}
