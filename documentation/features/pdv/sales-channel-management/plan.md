...
- **Depends/parallel:** Depends on all Builder diffs from F1–F5 being integrated; activate exactly one read-only [`Integrated Reviewer`](../../../agents/implementation-reviewer-agent.md) after integration and before readiness. The Reviewer may run alongside integrated sensors, but no per-Builder or per-phase Reviewer is created.
...
| Review | Complete integrated candidate, cross-Builder contracts and affected UI/server surfaces | All `RF-*`, `CA-*`, `MV-*`, `VIS-*` | [`Integrated Reviewer`](../../../agents/implementation-reviewer-agent.md) | Reviewer report verified in `./evaluation.md`; no blocking findings | `passed` |
