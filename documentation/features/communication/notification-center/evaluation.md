...
| `EV-01` | Cross-layer test integrity | `pnpm check:test-integrity` | Passed against the merged main baseline: 391 tracked test files and 37 changed test files; no Communication-center path violation was reported. | `passed` |
| `EV-02` | Core and Validation | `pnpm --filter @scoops/core test:coverage`; Core/Validation code and type checks | Core coverage passed: 88 files, 229 tests; statements 66.66%, branches 61.29%, functions 71.69%, lines 68.59%. Workspace Core/Validation code and types also passed. | `passed` |
...
| `EV-03` | Server | `pnpm --filter server test:coverage`; server code/types/build and focused controller, database, messaging, source-transaction suites | Merged-tree coverage exercised 88 files and 231 tests and cleared all configured floors: statements 73.37%, branches 54.54%, functions 71.81%, lines 76.63%. One unrelated timing-sensitive resend-invitation assertion failed in the full run; its isolated baseline test rerun passed. Server code/types/architecture/build and focused Communication/Identity/PDV tests passed. | `passed with baseline flake isolated` |
| `EV-04` | Web | `pnpm --filter web test:coverage`; Web code/types and focused widget suites | Web coverage passed: 181 files, 497 tests; statements 59.21%, branches 58.45%, functions 56.12%, lines 60.85%. Web code/types, focused widget tests, and focused Chromium route checks passed. | `passed` |
...
| `EV-05` | Web browser integration | `pnpm --filter web exec playwright test tests/routes/communication/notifications.index.test.tsx --project=chromium --workers=1` | Focused Chromium route coverage passed on the merged tree: the isolated pagination retry passed 1/1 after a transient first run (5/6), preserving the 6-test suite’s behavior and screenshot assertions. | `passed after transient rerun` |
| `EV-05-A` | Web browser integration | `pnpm --filter web exec playwright test tests/routes --workers=1` | CI-equivalent route gate passed: 200 tests. Existing route-boundary console diagnostics remained confined to tests intentionally exercising invalid/error states; no unexpected failed requests were reported. | `passed` |
...
| `EV-06` | Cross-layer path conformance | `pnpm check:spec-implementation -- documentation/features/communication/notification-center/spec.md` | Passed against `main` baseline at Spec revision 4: 146 contracted paths (Create 90, Modify 52, Generate 4); obsolete root-level job/validator paths were corrected to the current `jobs/tests` and Inngest locations. Unrelated changed paths (32) were ignored by the sensor. | `passed` |
...
| `FND-006` | mainline integration | Required merge of `origin/main` moved shared outbox and transaction contracts to `EventsRepository`/Inngest | `EV-02`, `EV-03`, `EV-06`, `MV-03`, `MV-04` | `resolved` | Adapted source publishers, transaction-bound PDV stock consumption, Communication fixture and job barrel, Identity/PDV controller tests, and event-validation paths; removed the obsolete root-level validator and reran merged-tree types, code, architecture, coverage thresholds, focused Server tests, Inngest tests, and browser validation. |
...
| --- | --- | --- | --- | --- |
| `CI-01` | Server CI (push) | `df894b1a9a2c6d842ae8cafbd756baec6532a37f` | `failed — CI environment missing Mailpit for mainline email Inngest tests` | [run 34076484017](https://github.com/rafinel/scoops/actions/runs/34076484017) |
| `CI-02` | Server CI (pull request) | `df894b1a9a2c6d842ae8cafbd756baec6532a37f` | `failed — same Mailpit dependency; superseded by correction` | [run 34076517767](https://github.com/rafinel/scoops/actions/runs/34076517767) |
| `CI-03` | Core CI (push) | `df894b1a9a2c6d842ae8cafbd756baec6532a37f` | `passed` | [run 34076483999](https://github.com/rafinel/scoops/actions/runs/34076483999) |
| `CI-04` | Core CI (pull request) | `df894b1a9a2c6d842ae8cafbd756baec6532a37f` | `passed` | [run 34076517731](https://github.com/rafinel/scoops/actions/runs/34076517731) |
| `CI-05` | Validation CI (push) | `df894b1a9a2c6d842ae8cafbd756baec6532a37f` | `passed` | [run 34076484038](https://github.com/rafinel/scoops/actions/runs/34076484038) |
...
| `2026-09-06` | Mainline integration correction completed after `origin/main` moved shared messaging to `EventsRepository`/Inngest. Contract paths were updated to the active `jobs/tests` layout; the obsolete root-level event validator was removed. Merged-tree Core coverage passed with 88 files/229 tests; Server coverage cleared configured floors at 88 files/231 tests (73.62% statements, 54.98% branches, 72.00% functions, 76.90% lines), with one unrelated resend-invitation timing assertion passing on isolated rerun; Inngest passed 8 files/23 tests; focused Server tests passed 6 files/28 tests. |
