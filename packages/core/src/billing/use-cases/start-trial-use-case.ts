  ...
  async execute(request: StartTrialRequest): Promise<Subscription> {
    ...
    const subscription = await this.database.run(
      async ({
        ...
        const existingTrial = await trialEligibilitiesRepository.findByEmailHash(
          request.emailHash,
        )
        ...
        const existingSubscription = await subscriptionsRepository.findByEstablishmentId(
        ...
        await trialEligibilitiesRepository.add({
        ...
        const subscription = await subscriptionsRepository.add({
          establishmentId: request.establishmentId,
