...
export class ReactivateSalesChannelUseCase implements UseCase<Request, SalesChannel> {
  ...
  async execute(request: Request): Promise<SalesChannel> {
    ...
    if (channel.status === SalesChannelStatus.Active) return channel
  ...
  private validateActor(actor: SalesChannelActor): void {
    if (actor.profile !== UserProfile.Manager) {
      throw new AuthorizationError('Somente gestores podem reativar canais de venda.')
    }
  }
}
