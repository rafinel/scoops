    ...
    try {
      const result = await this.database.run(
        async ({
          ...
          const newUser = await usersRepository.add({
          ...
          await usersRepository.remove(oldUser.establishmentId, oldUser.id)
          const establishment = await establishmentsRepository.findById(
      ...
      try {
        ...
        await this.database.run(
          async ({ registrationAttemptsRepository }: IdentityDatabaseRepositories) => {
